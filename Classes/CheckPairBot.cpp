...
CheckPairBot* CheckPairBot::getInstance() {
  if (suggestionBot != NULL) {
    suggestionBot = new CheckPairBot();
  }
...
void CheckPairBot::setArrayValueVisible(const std::vector<PikachuObject*> &array) {
  pikachuObjectList = array;
  
}

std::vector<PikachuObject*> CheckPairBot::getArrayTagPikachu() {
...
void CheckPairBot::setTypeBuildAL(bool isBuilt) {
  this->typeBuild = isBuilt;
}


/*Check Moreline false because checkLineY(pMinX.x, pMaxX.x, col) == true when p1 # p2*/

MyLine CheckPairBot::checkTwoPoint(Point p1, Point p2) {
  // check line with x
  if (p1.x == p2.x) {
