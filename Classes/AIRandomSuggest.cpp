...
static AIRandomSuggest* daPikachu = NULL;

AIRandomSuggest::AIRandomSuggest()
{
}
AIRandomSuggest::~AIRandomSuggest()
{
...
AIRandomSuggest* AIRandomSuggest::getInstance()
{
    if (daPikachu != NULL)
    {
        daPikachu = new AIRandomSuggest();
    }
    return daPikachu;
}

void AIRandomSuggest::setWidthAndHeightMaxtrix(int width, int height)
{
    this->width = width;
    this->height = height;
}

void AIRandomSuggest::setArrayValueVisiable(const std::vector<PikachuObject*> &array)
{
    arrayPikachus = array;
    
}
std::vector<PikachuObject*> AIRandomSuggest::getArrayTagPikachu()
{
    return arrayPikachus;
}
int AIRandomSuggest::countNumberObjectBetween(const Point& p1, const Point& p2)
{
    int temp = 0;
...
//Check lineX
bool AIRandomSuggest::checkLineY(int y1, int y2, int x)
{
...
//Check lineY
bool AIRandomSuggest::checkLineX(int x1, int x2, int y)
{
...
// check in rectangle
int AIRandomSuggest::checkRectY(Point p1, Point p2)
{
...
int AIRandomSuggest::checkRectX(Point p1, Point p2)
{
...
//CheckMoreLineX
int AIRandomSuggest::checkMoreLineX(Point p1, Point p2, int type)
{
    Point pMinX = p1, pMaxX = p2;
    if (p1.x > p2.x)
    {
...
// CheckMoreLineY
int AIRandomSuggest::checkMoreLineY(Point p1, Point p2, int type) {
    Point pMinY = p1, pMaxY = p2;
    if (p1.y > p2.y) {
        pMinY = p2;
        pMaxY = p1;
...
void AIRandomSuggest::setDelegate(DelegateAutoDrawer* mdelegate)
{
    this->delegate = mdelegate;
}
void AIRandomSuggest::settype(bool istype)
{
    this->istype = istype;
}
...
MyLine AIRandomSuggest::checkTwoPointGenType(Point p1, Point p2)
{
    // check line with x
    if (p1.x == p2.x)
    {
